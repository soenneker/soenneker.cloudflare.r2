...
{
    public async ValueTask<R2ListCustomDomains200?> ListCustomDomains(string accountId, string bucketName, string? apiKey = null, CancellationToken cancellationToken = default)
    {
    ...
    public async ValueTask<R2EditCustomDomainSettings200?> UpdateCustomDomain(string accountId, string bucketName, string domain,
        R2EditCustomDomainRequest request, string? apiKey = null, CancellationToken cancellationToken = default)
    {
        CloudflareOpenApiClient client = await GetClient(apiKey, cancellationToken).NoSync();
    ...
    public async ValueTask<R2GetBucketPublicPolicy200?> GetManagedDomain(string accountId, string bucketName, string? apiKey = null, CancellationToken cancellationToken = default)
    {
        CloudflareOpenApiClient client = await GetClient(apiKey, cancellationToken).NoSync();
